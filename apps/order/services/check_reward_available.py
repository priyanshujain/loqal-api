...
from apps.rewards.dbapi import (get_cash_rewards,
                                get_voucher_rewards,)
...
class CheckRewardAvailable(ServiceBase):
    ...
    def handle(self):
        cash_rewards = get_cash_rewards(consumer_id=self.consumer_id, merchant_id=self.merchant_id)
        if cash_rewards.exists():
            return self._cash_rewards_details(cash_rewards)
        else:
            voucher_rewards = get_voucher_rewards(consumer_id=self.consumer_id, merchant_id=self.merchant_id)
    ...
    def _cash_rewards_details(self, cash_rewards):
        cash_reward = cash_rewards.aggregate(total_available=Sum("available_value"))
